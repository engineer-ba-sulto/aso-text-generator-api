### チケット: P2-003 このバージョンの最新情報 (4,000 文字) 生成

#### 背景 / 目的

- **背景**: タイトル生成機能は実装済みだが、主要キーワードを自然に組み込んだ 4,000 文字以内の最新情報テキストを生成する機能が未実装
- **目的**: 日本語と英語の定型文テンプレートを準備し、リクエストされた言語に応じて主要キーワードを自然に組み込んだ最新情報テキストを生成する
...
- **最新情報生成アルゴリズムの実装**
  ...
  ```python
  # app/services/whats_new_generator.py
...
- 主要キーワードが 4〜7 回程度自然に組み込まれること
- 生成された最新情報が 4,000 文字以内であること
- 日本語・英語の両方で適切に生成されること
- アプリの特徴が適切に組み込まれること
- テンプレートに従った構造になっていること
- 禁止文字が含まれていないこと
- エラーハンドリングが適切に動作すること
...
- 追加:
  - `app/services/whats_new_generator.py` (最新情報生成サービス)
  - `tests/test_whats_new_generator.py` (最新情報生成テスト)
- 変更:
  - `app/services/text_generator.py` (最新情報生成機能の統合)
  - `app/models/response_models.py` (最新情報レスポンスモデルの追加)

#### 依存関係 / ブロッカー

- **依存**: P2-002（タイトル生成）の完了
- **ブロッカー**: なし
- **後続**: ステップ 3（Gemini 連携とクリエイティブテキスト生成機能）は本チケットの完了に依存

#### 参考資料

- `docs/MVP開発ロードマップ.md` の「2-3. このバージョンの最新情報 (4,000 文字) 生成」
- `docs/ASO戦略ルールまとめ.md` の最新情報生成ルール
- [App Store Connect ヘルプ](https://help.apple.com/app-store-connect/)

#### リスク / 留意点

- **文字数制限**: 4,000 文字制限の厳密な遵守
- **キーワード自然性**: 主要キーワードの自然な組み込み
